...
uint8_t movementManager();         		// Makes the kilobot move depending on its roles and checks if it is winner
...
uint8_t checkIfWinner(uint8_t collision);		// Returns true if a catcher catches a runner, false if a runner get caught, -1 otherwie (meaning that the game is going on) 
